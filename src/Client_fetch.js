...
function myFetch(method, url, body, cb, headers2, err_callback) {
  ...
  if (headers2) {
    ...
    headers = { 'Content-Type': 'application/json',cookie: 'csrftoken='+csrftoken };
  }
  if (method === 'GET') {
    data = {
      method: method,
...
function postForm(url, data, cb,err_callback) {
  ...
  return fetch(host+url, {
    headers: { 'content-type': 'application/x-www-form-urlencoded',cookie: 'csrftoken='+csrftoken },
    method: method,
    credentials: 'include',
    body: data,
  }).then((response)=>{
...
function login_index(cb) {
  return get('/rest/login_index', undefined, cb);
}
function logout(cb) {
...
function login(username, password, cb) {
  login_index(res => {
    ...
    postForm("/rest/login",data, cb,(err)=>{
        console.log(err);
      })
    }
  });
